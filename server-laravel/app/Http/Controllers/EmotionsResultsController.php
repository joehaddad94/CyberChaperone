...
class emotionsResultsController extends Controller {
    ...
    public function fetchDailyDataAnalysis(Request $request) {
        ...
        try {
            ...
            //Calculating the average emotions percentage
            $averageEmotions = [];
            ...
            // Calculating max percentage of each emotion
            $maxEmotions = [];

            if ($totalEmotions > 0) {
                foreach ($results[0]['emotionPercentage'] as $emotion => $percentage) {
                    $maxEmotions[$emotion] = $percentage;
                }

                foreach ($results as $result) {
                    foreach ($result['emotionPercentage'] as $emotion => $percentage) {
                        if ($percentage > $maxEmotions[$emotion]) {
                            $maxEmotions[$emotion] = $percentage;
                        }
                    }
                }
